...
def test_batch_calculate_trading_vs_natural_days():
    ...
    try:
        ...
        svc.record_buy_transaction(symbol, quantity=10, price=99.0, transaction_date="2024-01-04")

        # only_trading_days = True → 仅两天（1/05, 1/08）
        calc = PnLCalculator(storage=storage, only_trading_days=True)
        res_trading = calc.batch_calculate_historical_pnl("2024-01-05", "2024-01-08", [symbol])
        ...
        res_natural = calc2.batch_calculate_historical_pnl("2024-01-05", "2024-01-08", [symbol])
...
def test_portfolio_summary_values():
    ...
    try:
        ...
        svc = TransactionService(storage)
        svc.record_buy_transaction(symbol, quantity=10, price=100.0, transaction_date="2024-01-09")

        portfolio = PortfolioService(storage)
        summary = portfolio.get_portfolio_summary(as_of_date="2024-01-10")
...
def test_transaction_validation_excessive_commission_raises():
    ...
    try:
        ...
        try:
            svc.record_buy_transaction(symbol, quantity=10, price=200.0, transaction_date="2024-01-10", commission=30.0)
...
def test_batch_calculate_symbols_filter():
    ...
    try:
        ...
        svc = TransactionService(storage)
        svc.record_buy_transaction(s1, quantity=1, price=10.0, transaction_date="2024-02-01")
        svc.record_buy_transaction(s2, quantity=1, price=20.0, transaction_date="2024-02-01")

        calc = PnLCalculator(storage=storage)
        res = calc.batch_calculate_historical_pnl("2024-02-01", "2024-02-02", symbols=[s1])
        assert res['symbols_processed'] == 1

        # 检查 daily_pnl 仅对 s1 写入
        rows_s1 = storage.get_daily_pnl(s1, "2024-02-01", "2024-02-02")
        rows_s2 = storage.get_daily_pnl(s2, "2024-02-01", "2024-02-02")
