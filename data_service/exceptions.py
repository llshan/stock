...
class DownloadError(DataServiceError):
    ...
    def __init__(self, message: str, symbol: Optional[str] = None, 
                 source: Optional[str] = None, retries: Optional[int] = None):
        context = {}
        if symbol:
...
class DataValidationError(DataServiceError):
    ...
    def __init__(self, message: str, symbol: Optional[str] = None, 
                 field: Optional[str] = None, expected: Optional[Any] = None, 
...
class StorageError(DataServiceError):
    ...
    def __init__(self, message: str, operation: Optional[str] = None,
                 table: Optional[str] = None, symbol: Optional[str] = None):
        context = {}
        if operation:
...
class DatabaseOperationError(StorageError):
    ...
    def __init__(self, message: str, operation: str, table: Optional[str] = None,
                 query: Optional[str] = None, symbol: Optional[str] = None):
        context = {'operation': operation}
        if table:
            context['table'] = table
        if query:
...
class DataQualityError(DataServiceError):
    ...
    def __init__(self, message: str, symbol: Optional[str] = None,
                 quality_issues: Optional[list] = None, quality_grade: Optional[str] = None):
        context = {}
        if symbol:
            context['symbol'] = symbol
        if quality_issues:
...
class InsufficientDataError(DataServiceError):
    ...
    def __init__(self, message: str, symbol: Optional[str] = None,
                 required_points: Optional[int] = None, actual_points: Optional[int] = None):
        context = {}
        if symbol:
            context['symbol'] = symbol
...
class TimeoutError(DataServiceError):
    ...
    def __init__(self, message: str = "Operation timed out", 
                 operation: Optional[str] = None, timeout_seconds: Optional[int] = None):
        context = {}
        if operation:
            context['operation'] = operation
        if timeout_seconds:
...
# 异常工厂函数，方便创建常见异常
def create_download_error(symbol: str, source: str, error_msg: str, retries: int = 0) -> DownloadError:
    """创建下载错误"""
    return DownloadError(f"Failed to download {symbol} from {source}: {error_msg}", 
                        symbol=symbol, source=source, retries=retries)


def create_network_error(symbol: str, url: str, status_code: int = None) -> NetworkError:
    """创建网络错误"""
...
def create_storage_error(operation: str, table: str, symbol: str = None, 
                        error_msg: str = None) -> StorageError:
    """创建存储错误"""
...
def create_validation_error(field: str, expected: Any, actual: Any, 
                           symbol: str = None) -> DataValidationError:
    """创建验证错误"""
    message = f"Validation failed for field '{field}': expected {expected}, got {actual}"
    if symbol:
        message = f"[{symbol}] {message}"
    
    return DataValidationError(message, symbol=symbol, field=field, 
