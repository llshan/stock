...
from .base_models import BaseDataModel, SummaryStats, calculate_summary_stats
...
class PriceData:
    ...
    def validate(self) -> List[str]:
        ...
        # 检查列表长度一致性
        lengths = [len(self.dates), len(self.open), len(self.high), 
                  len(self.low), len(self.close), len(self.volume), len(self.adj_close)]
        if len(set(lengths)) > 1:
            issues.append(f"数据长度不一致: {dict(zip(['dates', 'open', 'high', 'low', 'close', 'volume', 'adj_close'], lengths))}")
        ...
        # 检查价格逻辑性
        for i, (open_p, high_p, low_p, close_p) in enumerate(zip(self.open, self.high, self.low, self.close)):
...
class StockData:
    ...
    def validate(self) -> List[str]:
        ...
        if self.price_data.dates and len(self.price_data.dates) != self.data_points:
            issues.append(f"数据点数不匹配: 声明{self.data_points}个，实际{len(self.price_data.dates)}个")
...
def create_empty_stock_data(symbol: str, start_date: str, end_date: str, error_msg: str = None) -> Dict[str, Any]:
    """创建包含错误信息的空股票数据"""
    return {
        'symbol': symbol,
