...
class DataServiceConfig:
    ...
    @classmethod
    def from_env(cls, db_path: str = None) -> 'DataServiceConfig':
        """
        从环境变量创建配置
        ...
        # 下载器配置
        config.downloader.max_retries = int(os.getenv('DATA_SERVICE_MAX_RETRIES', config.downloader.max_retries))
        config.downloader.base_delay = int(os.getenv('DATA_SERVICE_BASE_DELAY', config.downloader.base_delay))
        config.downloader.timeout = int(os.getenv('DATA_SERVICE_TIMEOUT', config.downloader.timeout))
        
        # 批量配置
        config.batch.batch_size = int(os.getenv('DATA_SERVICE_BATCH_SIZE', config.batch.batch_size))
        config.batch.delay_between_requests = int(os.getenv('DATA_SERVICE_BATCH_DELAY', config.batch.delay_between_requests))
        
        # 日志配置
        config.log_level = os.getenv('DATA_SERVICE_LOG_LEVEL', config.log_level)
        config.enable_file_logging = os.getenv('DATA_SERVICE_ENABLE_FILE_LOG', 'false').lower() == 'true'
        config.log_file_path = os.getenv('DATA_SERVICE_LOG_FILE', config.log_file_path)
        
        # 默认开始日期
        config.default_start_date = os.getenv('DATA_SERVICE_DEFAULT_START_DATE', config.default_start_date)
        config.default_data_source = os.getenv('DATA_SERVICE_DEFAULT_SOURCE', config.default_data_source)
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> 'DataServiceConfig':
        ...
        # 日志配置
        for key in ['log_level', 'log_format', 'enable_file_logging', 'log_file_path']:
    ...
    def update(self, **kwargs) -> 'DataServiceConfig':
        """
        更新配置参数
        ...
        """
        # 支持直接更新下载器参数
        downloader_keys = ['max_retries', 'base_delay', 'timeout', 'rate_limit_delay']
        for key in downloader_keys:
            if key in kwargs:
                setattr(self.downloader, key, kwargs.pop(key))
        
        # 支持直接更新数据库参数
        database_keys = ['db_path', 'db_type', 'connection_timeout', 'max_connections', 'enable_foreign_keys']
        for key in database_keys:
            if key in kwargs:
                setattr(self.database, key, kwargs.pop(key))
        
        # 支持直接更新批量参数
        batch_keys = ['batch_size', 'delay_between_requests', 'max_concurrent_downloads', 'enable_progress_logging']
