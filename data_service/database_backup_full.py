    ...
    def store_financial_statements(self, symbol: str, financial_data: Union[FinancialData, Dict]):
        ...
        try:
            ...
            for statement_type, statement_data in statements.items():
                # Handle both FinancialStatement dataclass and dict format
                if isinstance(statement_data, FinancialStatement):
                    periods = statement_data.periods
                    items = statement_data.items
                elif isinstance(statement_data, dict) and 'error' not in statement_data:
    ...
    def store_data_quality(self, symbol: str, quality_data: Union[DataQuality, Dict]):
        ...
        try:
            ...
            if isinstance(quality_data, DataQuality):
                issues_json = json.dumps(quality_data.issues, ensure_ascii=False)
                values = (
                    symbol,
                    quality_data.stock_data_available,
                    quality_data.financial_data_available,
                    quality_data.data_completeness,
    ...
    def store_comprehensive_data(self, symbol: str, data: Union[ComprehensiveData, Dict]):
        ...
        try:
            ...
            if isinstance(data, ComprehensiveData):
                ...
                is_incremental = stock_data.incremental_update if stock_data else False
            else:
                stock_data = data.get('stock_data')
                financial_data = data.get('financial_data')
                data_quality = data.get('data_quality')
                is_incremental = stock_data.get('incremental_update', False) if isinstance(stock_data, dict) else False
            ...
            if stock_data:
                if isinstance(stock_data, StockData):
                    # 检查是否有新数据需要存储
                    if stock_data.price_data and stock_data.data_points > 0:
                        self.store_stock_prices(symbol, stock_data.price_data, incremental=is_incremental)
                        
                        # 记录价格数据下载日志
                        self.store_download_log(symbol, 'stock_prices', 'success', stock_data.data_points)
                    ...
                    # 检查是否有新数据需要存储
                    if price_data and stock_data.get('data_points', 0) > 0:
                        self.store_stock_prices(symbol, price_data, incremental=is_incremental)
                        
                        # 记录价格数据下载日志
                        self.store_download_log(symbol, 'stock_prices', 'success', stock_data.get('data_points', 0))
                    elif stock_data.get('no_new_data'):
                        self.logger.info(f"📊 {symbol} 无新数据需要更新")
                else:
                    # 记录失败日志
            ...
            if financial_data:
                if isinstance(financial_data, FinancialData):
                    self.store_financial_statements(symbol, financial_data)
                    
                    # 记录财务数据下载日志
                    statements_count = len(financial_data.financial_statements)
                    self.store_download_log(symbol, 'financial_data', 'success', statements_count)
                elif isinstance(financial_data, dict) and 'error' not in financial_data:
                    self.store_financial_statements(symbol, financial_data)
                    
                    # 记录财务数据下载日志
                    statements_count = len(financial_data.get('financial_statements', {}))
                    self.store_download_log(symbol, 'financial_data', 'success', statements_count)
                else:
                    # 记录失败日志
                    if isinstance(financial_data, dict):
                        error_msg = financial_data.get('error', '未知错误')
                    else:
                        error_msg = '未知错误'
            ...
            # 存储数据质量评估
            if data_quality:
                self.store_data_quality(symbol, data_quality)
...
if __name__ == "__main__":
    ...
    setup_logging()
    logging.getLogger(__name__).info("🗄️ 股票数据库测试")
    ...
    logging.getLogger(__name__).info("✅ 数据库测试完成")
