...
from .base_models import BasicInfo
...
class FinancialStatement:
    ...
    def get_item_value(self, item_name: str, period: Optional[str] = None) -> Optional[float]:
        """
        获取指定项目的值
        ...
        if income_stmt and balance_sheet:
            ...
            if current_liabilities and current_liabilities != 0:
                ratios['current_ratio'] = (
                    (current_assets / current_liabilities) if current_assets else None
                )
...
def create_empty_financial_statement(
    statement_type: str,
) -> FinancialStatement:
    """创建空的财务报表"""
    return FinancialStatement(statement_type=statement_type, periods=[], items={})


def create_empty_financial_data(symbol: str, error_msg: Optional[str] = None) -> Dict[str, Any]:
    """创建包含错误信息的空财务数据"""
    return {
        'symbol': symbol,
...
def merge_financial_statements(
    old_stmt: FinancialStatement, new_stmt: FinancialStatement
) -> FinancialStatement:
    """
    合并财务报表（用于数据更新）
    ...
    return FinancialStatement(statement_type=new_stmt.statement_type, periods=periods, items=items)
