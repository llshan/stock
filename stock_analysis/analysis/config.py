...
class PipelineConfig:
    ...
    enabled_operators: List[str] = field(default_factory=lambda: ['ma', 'rsi', 'drop_alert'])
    # 跌幅预警参数
...
class Config:
    ...
    def ensure_directories(self) -> None:
        """确保必需的目录存在"""
        Path(self.app.results_dir).mkdir(parents=True, exist_ok=True)
        Path(self.app.charts_dir).mkdir(parents=True, exist_ok=True)
...
def save_config_to_file(config: Config, filepath: str) -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).info(f"配置已保存到: {out_path}")
    except Exception as e:
